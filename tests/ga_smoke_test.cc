...
#include "teststringGenerator/strategies/always_valid_validator.hh"
...
{
    return GeneticGenerator(config,
                            make_unique<BaselineGaFactory>(),
                            make_unique<RandomPopulationInitializer>(config.minLength,
                                                                     config.maxLength),
...
int main()
{
    ...
    // Every name must resolve to a real block. This is the property that
    // actually matters downstream: genATC throws on an unknown name.
    const set<string> declared = [&spec] {
        set<string> names;
